...
import { error, log, warning } from '../../utils/logging';
...
export type ListConfigParamOptions = EditConfigParamOptions;

function validateOptions(
  options:
    | EditConfigParamOptions
    | GetConfigParamOptions
    | SetConfigParamOptions
) {
  if (!options.global) {
...
export async function listConfig(options: ListConfigParamOptions) {
  validateOptions(options);
  const configFile = await getConfigFile(options.config);
...
export async function setConfigParam(options: SetConfigParamOptions) {
  ...
  log(
    keyExists ? 'UPDATE' : 'CREATE',
    `Set option ${options.key} to ${options.value}`
  );
}
